; $Id: rot.pro,v 1.6 1998/01/15 18:43:53 scottm Exp $
;
; Copyright (c) 1982-1998, Research Systems, Inc.  All rights reserved.
;       Unauthorized reproduction prohibited.

;Rotate and magnify an image
FUNCTION IDP3_ROT, A, ANGLE, MAG, xrot, yrot, INTERP = interp, $
        MISSING = missing, PIVOT = pivot, CUBIC = cubic, pixdef = pixdef
;+
; NAME:
;	ROT
;
...
;
; CALLING SEQUENCE:
;	Result = IDP3_ROT(A, Angle, [Mag, X0, Y0], MISSING = missing,
;		INTERP = Interp, CUBIC = Cubic, pixdef = pixdef)
;
; INPUTS:
...
;       
;       PIXDEF: Definition of the origin of the first pixel, either its center
;               (value=0) or its lower left corner (value=1).  If lower left
;               corner is specified 0.5 is subtracted from rotation center
;               coordinates for compatibility with rot.
;
; KEYWORDS:
...
;       Sep, 1999.      Changed angle conversion to double precision, pixdef
;                       added - E.B. Stobie
;-
;
;
...
;
        if n_elements(pixdef) le 0 then pixdef=0
...
;
	...
	if n_elements(missing) eq 0 $
           then	return, poly_2d(a, cx, cy, i, CUBIC=cubic) $
           else return, poly_2d(a, cx, cy, i, MISSING = missing, CUBIC=cubic)
	;else begin
	;  good = where(a ne missing)
	;  return, poly_2d(a[good],cx,cy, i, MISSING = missing, CUBIC=cubic)
        ;endelse
END
