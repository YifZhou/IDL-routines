...
FUNCTION UA_ROT,A,ANGLE,MAG,X0,y0, INTERP = interp, MISSING = missing, $
        PIVOT = pivot, CUBIC = cubic, pixdef = pixdef
;+
; NAME:
;	ROT
;
...
;
; CALLING SEQUENCE:
;	Result = UA_ROT(A, Angle, [Mag, X0, Y0], MISSING = missing,
...
;
	...
	if n_elements(missing) eq 0 then $
		return,poly_2d(a,cx,cy, i, CUBIC=cubic) $
