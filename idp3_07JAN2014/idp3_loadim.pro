...
@idp3_errors
  ...
  filenames = Dialog_Pickfile(/Read, /multiple_files, Get_Path=outpath, $
	     Path=inpath, Filter=infilt, Title='Please Select Image File')
  cinfo.imagepath = outpath
  print, filenames
  ename = cinfo.extnam
  for ijk = 0, n_elements(filenames) - 1 do begin
    filename = strtrim(filenames(ijk), 2)
    exnum = -1
    ...
    if fcount gt 0 then begin
      ...
      Case lextn of
	   ...
	   then nextn = 0
	     ...
	     str =  'LoadImage: Error in extension name'
	     ...
	     loaddata = 0
	       ...
	       if maxver gt 1 then begin
	         str = 'LoadImage: Loading ' + ename + ' extension ' + $
	         strtrim(string(exread),2) + $
	         ' which is sample ' + strtrim(string(exnum),2) + ' (time order)'
                 idp3_updatetxt, cinfo, str
               endif
             endif else begin
	       ...
	       exread = exnum < maxver
             endelse
	     fits_read, fcb, tempdata, ihdr, Extver=exread, Extname=ename, $
	       /NO_PDU, /no_abort
	   ...
	   imsz = size(tempdata)
	   psz = size(phdr)
	   if imsz[0] eq 0 and psz[0] eq 0 then begin
	     str = 'LoadImage: No data in file'
	     idp3_updatetxt, cinfo, str
	     return
	     ...
	     if exnum lt 0 then begin
	       title = 'Enter plane of 3-D image'
	       label = '0-' + strtrim(string(imsz[3]-1),2) + ', *'
	       valstr = idp3_getvals(title, pln, groupleader=info.idp3Window, $
		      lab1=label, cancel=cancel, xp=200, yp=200)
               if cancel eq 1 then begin
	         str = 'LoadImage: Nothing loaded'
	         idp3_updatetxt, cinfo, str
	         return
               endif
               all = strpos(valstr, '*')
      ...
      '.tif': Begin
       ; this is a tiff file
       stat = query_tiff(filename, tifinfo)
       nimages = tifinfo.num_images
       dim = tifinfo.dimensions
       chans = tifinfo.channels[0]
       if nimages gt 1 or n_elements(dim) gt 2 or chans gt 1 then begin
         if n_elements(dim) eq 3 then begin
	   title = 'Enter plane of 3-D image' 
	   ilim = dim[0]
         endif else if nimages gt 1 then begin
           title = 'Enter images to load'
	   ilim = nimages
         endif else begin
	   title = 'Enter channels to load'
	   ilim = chans
         endelse
         label = '0-' + strtrim(string(ilim-1),2) + ', *'
         valstr = idp3_getvals(title, pln, groupleader=info.idp3Window, $
		  lab1=label, cancel=cancel, xp=200, yp=200)
         if cancel eq 1 then begin
	   str = 'LoadImage: Nothing loaded'
	   ...
	   if rng gt 0 then begin
	     len = strlen(valstr)
	     findx = fix(strmid(valstr, 0, rng)) > 0
	     lindx=fix(strmid(valstr, rng+1, len-rng)) > findx < (ilim-1)
           endif else begin
	     findx = fix(valstr) > 0 < (ilim-1)
	     lindx = findx
           endelse
         endelse
       endif
       ...
       if nimages eq 1 then begin
         ...
         if n_elements(dim) gt 2 or chans gt 1 then begin
           imsz = size(tempdata)
           if imsz[0] eq 3 then begin
	     if tifinfo.bits_per_sample eq 8 $
	       then tmp = bytarr(imsz[2],imsz[3],imsz[1]) $
	       else tmp = uintarr(imsz[2],imsz[3],imsz[1])
             tmp[*,*,0] = tempdata[0,*,*]
             tmp[*,*,1] = tempdata[1,*,*]
             tmp[*,*,2] = tempdata[2,*,*]
             tempdata = float(tmp)
...
;       if tifinfo.bits_per_sample eq 8 $
;	  then tempdata = bytarr(dim[0],dim[1],nimages)$
;	  else tempdata = uintarr(dim[0],dim[1],nimages)
         tempdata = fltarr(dim[0],dim[1],nimages)
         for ti = 0, nimages-1 do begin
	   tmpim = read_tiff(filename, image_index=ti)
	   tempdata[*,*,ti] = float(tmpim)
         endfor
         tmpim = 0
       endif
       loaddata = 1
