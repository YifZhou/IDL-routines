  ...
  case event.id of
	...
	idp3_selectval, event.top, 'Do you wish to overwrite existing file?',$
      ...
      if savfil eq 1 then begin
        ofile = 'idp3_clipmedpar.txt'
        ims = (*cmedianinfo.info.images)
	xo = cmedianinfo.info.sxoff
	yo = cmedianinfo.info.syoff
	idp3_mpar, ims, ofile, xo, yo
        text = 'idp3_clipmedian: ' + ofile + ',' + nfile
        idp3_updatetxt, cmedianinfo.info, text
	sig1 = cmedianinfo.info.negsig
	if sig1 gt 0. then sig1 = sig1 * (-1.0)
	sig2 = cmedianinfo.info.possig
	sfits = cmedianinfo.info.sfits
	pixorg = cmedianinfo.info.pixorg
	mdioz = cmedianinfo.info.mdioz
	mddz = cmedianinfo.info.mddz
	zoomflux = cmedianinfo.info.zoomflux
	iz = cmedianinfo.info.exclude_invalid
	iv = cmedianinfo.info.invalid
;	er = cmedianinfo.info.nsig_3d
	er = 0
        idp3_clipmedian,ofile,nfile,sfile,iz,iv,sig1,sig2,pixorg,mdioz,mddz,$
	   zoomflux, sfits, er
...
;    Widget_Control, cmedianinfo.selectfile, set_value=fpath
    Widget_Control, cmedianinfo.selectfile, set_value=Pathvalue
    end
...
@idp3_errors
  ...
  selectfile = Widget_Text  (med1base, Value = path, XSize = 80, /Edit)
  ...
  XManager, "idp3_clipmedpar", cmedbase, Event_Handler = "clipmedian_ev"
