pro idp3_cntrd, img, x, y, xcen, ycen, fwhm, coords, SILENT=silent, DEBUG=debug
;+
;  NAME: 
;	IDP3_CNTRD (CNTRD from Astron Library with few modifications)
;  PURPOSE:
;	Compute the centroid coordinates of a stellar object 
;	using the algorithm in the DAOPHOT FIND subroutine.
;
;  CALLING SEQUENCE: 
;	IDP3_CNTRD, img, x, y, xcen, ycen, [ fwhm , coords, /SILENT, /DEBUG]
;
;  INPUTS:     
;	IMG - Two dimensional image array
;	X,Y - Scalar or vector integers giving approximate stellar center
;       COORDS - coordinates of region used for computation
;
;  OPTIONAL INPUT:
...
;   Compute size of box needed to compute centroid
 ...
 nhalfbig = nhalf +3
 ;Extend box 3 pixels on each side to search for max pixel value
 nbig = nbox + 6      
 npts = N_elements(x) 
...
;  Find Y Centroid
 ...
 coords = [xmax-nhalf, xmax+nhalf, ymax-nhalf, ymax+nhalf]
 return
 end
