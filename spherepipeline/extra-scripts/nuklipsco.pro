function nuKLIPsco, cube,  k_klip, target=target, checkpoint = checkpoint, anglemask=anglemask, $
...
for ii=0, nspots-1 do begin
 print, '--[ NUKLIP  ]-- Processing Spot #',ii
   ...
   ;if nspots gt 1 then arcdist = (asin( 1.0 *(1.22 * (wl *(10.^(-6.)))/diam/!DTOR *3600./pixelscale ) / (rho[ii])) )/!DTOR ; Gives the minimum posang difference
	;if nspots le 1 then arcdist = (asin( 1.0 *(1.22 * (wl *(10.^(-6.)))/diam/!DTOR *3600./pixelscale ) / (rho)) )/!DTOR ; Gives the minimum posang difference
...
;for ii=0, nrings-1 do begin
   ...
   arcdist = (asin( 1.0 *(1.22 * (wl *(10.^(-6.)))/diam/!DTOR *3600./pixelscale ) / (annmode_inout[0]+(abs(annmode_inout[1]-annmode_inout[0] )/2.)) ))/!DTOR ; Gives the minimum posang difference
                                ; required to have a 1.5 FWHM arc
                                ; difference in H-band with a
                                ; diffraction limited 8m telescope at
                                ; the mid-point of the iith ring with
...
;      nottarget =  where(findgen(nimages) ne target) ; AD
	...
	if jj eq 0 then print, 'Annmode radius = ',(annmode_inout[0]+(abs(annmode_inout[1]-annmode_inout[0] )/2.))
      if N_elements(nottarget) lt 4 then begin
          ...
          print, ' -[ NUKLIP ]- Less than 4 frames avialable for PCA on annulus –  - Removing angular distance constraint'
...
for ii=0, nrings-1 do begin
   ...
   arcdist = (asin( 1.0 *(1.22 * (wl *(10.^(-6.)))/diam/!DTOR *3600./pixelscale ) / ((float(ii)+0.5)*wr )) )/!DTOR ; Gives the minimum posang difference
                                ; required to have a 1.5 FWHM arc
                                ; difference in H-band with a
                                ; diffraction limited 8m telescope at
                                ; the mid-point of the iith ring with
...
;      nottarget =  where(findgen(nimages) ne target) ; AD
      ...
      if N_elements(nottarget) lt 4 then begin
           nottarget =  where(findgen(nimages) ne target)
          if jj eq 0 then print, ' -[ NUKLIP ]- Less than 4 frames avialable for PCA on annulus – ',ii,' - Removing angular distance constraint'
