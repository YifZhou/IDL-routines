...
#  ESO, plus some additionnal custome routines written in IDL.
#
#  It has been designed to be very simple to use, with as little as
...
#  the beginning of the script. The data should not be compressed (.Z
#  or .gz extension), and the file name you be in the standard archive
#  form (e.g. SPHER.2015-02-03T06:47:27.430.fits)
#
...
#  message if it notices some potential problems. Any IFS sequence
#  will require the following files and calibrations:
#
...
#  missing, read the explanation carefully. After the sanity check the
#  script automatically exits and you have to set DO_SANITY_CHECK=0 to
#  proceed with the data reduction.
#
#  The second step is to create the basic calibrations:
#  dark/backgrounds, detector flats, spectra position, wavelength
#  calibration and IFU flat. This is done by switching the
#  corresponding keywords to 1 (e.g. DO_DET_DARK=1) and run the
#  script. If this is your first reduction, I advise to run them one
#  at the time. For most of the basic calibrations, the ESO pipeline
#  is used by running the appropriate recipe with esorex. The only
#  exception is the detector flats, which are created with a custom
#  IDL routine. All the calibrations are saved in the calib/
#  sub-directory. Don't forget to switch all basic calibration
#  keywords to 0 once they are done to avoid recreating them when
#  running the script again.
#
#  The following step is to run the pre-processing of the raw science
#  files. This is done with the custom IDL routine
#  sph_ifs_preprocess(). There are several options for this
#  pre-processing, which are triggered by the DO_COLLAPSE, DO_BKG_SUB,
#  DO_BADPIXEL and DO_CROSSTALK keywords. The effect of these options
#  are documented in the header of the sph_ifs_preprocess()
#  routine. Brief summary:
#
#    * DO_COLLAPSE: performe a temporal binning of the data
...
#  correction).
#
#  For this pre-processing step, there are two additional keywords:
#
#    * USE_SKY: force the use of sky backgrounds if both instrumental
#               dark/backgrounds and sky backgrounds are
#               available. This can provide slightly better cosmetics
#               in some cases
#    * WAFFLE_OBS: set this keyword to 1 if your sequence includes
#                  only star center (C) frames. The reason is that in
#                  a more classical sequence where you would have only
#                  1 or 2 star center frames to just calibrate the
#                  center, these frames will automatically be
#                  collapsed to increase the SNR of the satellite
#                  spots
#
#  After the pre-processing, the last step is to produce the
#  (x,y,lambda) science cubes using as input the pre-processed raw
#  files. This is done by setting DO_SCI_CORO=1 and running the script
#  (don't forget to set DO_PREPROC=0 to avoir redoing the
#  pre-processing). In this step, the ESO pipeline is run to generate
...
#  complete procedure is described in Vigan et al. 2015 (submitted)
#  and is included in the small data_reduction_ifs.pro IDL program
...
#  all straighforward. I hope that this script will simply some of
#  these steps and will allow to quickly obtain results. If you
#  encounter any problem using this tool, please do not hesiate to
#  contact me.
#
...
#
#  arthur.vigan - 08/2015 - added sanity check, commented for
#                 distribution
#
#  arthur.vigan - 07/2015 - added keyword for waffle observations,
#                added background specific for calibrations to improve
#                cosmetics
#
#  arthur.vigan - 05/2015 - updated for latest version of ESO pipeline
#
#  arthur.vigan - 11/2014 - several small improvements including
#                 automatic classification of files
#
#  arthu.vigan - 10/2014 - first usable version with main features
#
# AUTHOR:
# 
#   Arthur Vigan
...
DO_SANITY_CHECK=0      # sanity checks
...
# pre-processing
DO_PREPROC=1           # perform pre-processing
...
COLLAPSE_TYPE=mean      # collapse type: mean, angle, coadd
COLLAPSE_VAL=4          # collapse parameter (for mean, angle and coadd)
COLLAPSE_TOL=0.05       # collapse parameter tolerance (for mean and angle) 

USE_SKY=0               # if available, use preferably a sky instead of a dark/background

WAFFLE_OBS=0            # set to 1 if observations were all done with waffle
...
cd ${ROOT}raw/
dfits *.fits | fitsort dpr.type ins2.comb.ifs det.seq1.dit det.ndit
cd ${ROOT}
echo
echo
...
flat_whit_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT | grep CAL_BB_2_${MODE}  | awk '{print $1}'`)
...
flat_1020_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT | grep CAL_NB1_1_${MODE} | awk '{print $1}'`)
...
flat_1230_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT | grep CAL_NB2_1_${MODE} | awk '{print $1}'`)
...
flat_1300_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT | grep CAL_NB3_1_${MODE} | awk '{print $1}'`)
...
flat_1550_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT | grep CAL_NB4_2_${MODE} | awk '{print $1}'`)
...
spec_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep SPECPOS,LAMP | grep ${OBS} | awk '{print $1}'`)    
...
wave_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep WAVE,LAMP | grep ${OBS} | awk '{print $1}'`)
...
#
echo '* IFU flat files:'
ifu_files=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep FLAT,LAMP | grep ${OBS} | awk '{print $1}'`)
...
#
echo '* PSF files:'
sci_files_psf=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep OBJECT,FLUX | awk '{print $1}'`)
...
sci_files_cen=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep OBJECT | grep OBJECT,CENTER | awk '{print $1}'`)
...
sci_files_cor=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit ins2.comb.ifs | grep OBJECT | grep -v OBJECT,FLUX | grep -v OBJECT,CENTER | awk '{print $1}'`)
...
if [ $DO_SANITY_CHECK -ne 0 ]; then
    if [ ${#sci_files_cor[@]} -eq 0 ]; then
        echo '  Warning: there are no science (O) files. If this is a full sequence with waffle on, remember to set WAFFLE_OBS=1 for the analysis'
    fi
fi
echo
...
DIT=1.6
dark_files_cal=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep DARK | grep "${DIT}" | awk '{print $1}'`)
...
dark_files_psf=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep DARK | grep "${DIT}" | awk '{print $1}'`)
...
dark_files_cen=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep DARK | grep "${DIT}" | awk '{print $1}'`)
...
dark_files_cor=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep DARK | grep "${DIT}" | awk '{print $1}'`)
...
DIT=`dfits ${ROOT}raw/${sci_files_psf[0]}.fits | fitsort det.seq1.dit | grep -v FILE | awk '{print $2}'`
sky_files_psf=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep SKY | grep "${DIT}" | awk '{print $1}'`)
...
DIT=`dfits ${ROOT}raw/${sci_files_cen[0]}.fits | fitsort det.seq1.dit | grep -v FILE | awk '{print $2}'`
sky_files_cen=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep SKY | grep "${DIT}" | awk '{print $1}'`)
...
DIT=`dfits ${ROOT}raw/${sci_files_cor[0]}.fits | fitsort det.seq1.dit | grep -v FILE | awk '{print $2}'`
sky_files_cor=(`dfits ${ROOT}raw/SPHER*.fits | fitsort dpr.type det.seq1.dit | grep SKY | grep "${DIT}" | awk '{print $1}'`)
