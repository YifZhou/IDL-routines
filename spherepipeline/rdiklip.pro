...
obj=obj,nrings=nrings,wr =wr,n_ang =n_ang,annmode_inout=annmode_inout,spot_radius=spot_radius,rho=rho,phi=phi
...
;        zones = getzone(cube, annulus, angles)  ; Returns a nz x npix 2D array with the pixels in the given zone across the cube
      ...
      nottarget  =  findgen(nrefimages);where( abs(posang(frames)-posang(target))  ge ANGSEP*abs(arcdist) ) ; AD
